...
* Command 18 "Read Alarm State Level 1" also resets the alarm (even if it is latched).
  Likely command 19 "Read Alarm State Level 2" does the same.
  Resetting an alarm while leaving the chiller in standby can cause a serious problem, due to a major design flaw:
  ...
  * If an alarm occurs that puts the chiller in standby and the alarm is reset by command 18 during this 1.5 seconds, the user interface code will display a "mode mismatch alarm" which requires power cycling the chiller.
  ...
  Note that the chiller should already be in standby due to the alarm, so should not reduce chilling.
